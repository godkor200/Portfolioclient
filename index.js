...
for (let prop in DIGIT_VALUES) {
    if(romanNumeral.length === 1 && romanNumeral[0] === prop ){
    return DIGIT_VALUES[prop]
}



    if (romanNumeral[0] === prop) {
      var Num1 = DIGIT_VALUES[prop];
    }
    if (romanNumeral[1] === prop) {
      var Num2 = DIGIT_VALUES[prop];
    }
